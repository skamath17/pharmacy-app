...
if ($aiCommits.Count -eq 0) {
    $report += "`n*No commits found matching AI-related keywords.*`n`n"
    $report += "**Note:** This script identifies AI-generated code by searching commit messages, author names, and email addresses for keywords like: `cursor`, `ai`, `assistant`, `generated`, etc.`n`n"
    $report += "To improve tracking, consider:`n"
    $report += "- Adding `[AI]` or `[Cursor]` prefix to commit messages for AI-generated code`n"
    $report += "- Using a consistent commit message format`n"
    $report += "- Adding code comments like `// Generated by Cursor` or `<!-- AI-assisted -->`"
} else {
    $report += "`n| Hash | Author | Date | Message | Files | Lines +/- | Keywords |`n"
    $report += "|------|--------|------|---------|-------|-----------|----------|`n"
    ...
    foreach ($commit in $aiCommits) {
        ...
        $report += "| [$($commit.Hash)]($(git config --get remote.origin.url -replace '\.git$', '')/commit/$($commit.FullHash)) | $($commit.Author) | $($commit.Date.Split(' ')[0]) | $message | $($commit.FilesChanged) | $linesChange | $($commit.Keywords) |`n"
    }
    
    if ($Detailed) {
        $report += "`n---`n`n## Detailed Commit Information`n`n"
        foreach ($commit in $aiCommits) {
            $report += "### Commit: $($commit.Hash)`n`n"
            $report += "- **Author:** $($commit.Author) <$($commit.Email)>`n"
            $report += "- **Date:** $($commit.Date)`n"
            $report += "- **Message:** $($commit.Message)`n"
            $report += "- **Files Changed:** $($commit.FilesChanged)`n"
            $report += "- **Lines Added:** $($commit.LinesAdded)`n"
            $report += "- **Lines Deleted:** $($commit.LinesDeleted)`n"
            $report += "- **Matched Keywords:** $($commit.Keywords)`n`n"
            
            # Get file list for this commit
            $files = git diff-tree --no-commit-id --name-only -r $commit.FullHash
            if ($files) {
                $report += "**Files Modified:**`n"
...
if ($aiCommits.Count -gt 0) {
    Write-Host "`nTop AI Contributors:" -ForegroundColor Cyan
